...
public class NotificationDispatchService {
    ...
    public void ticketAssigned(Ticket ticket, Long actorUserId) {
        ...
        publish(
                ...
                "Ticket assigned",
                "Ticket #%d was assigned to %s"
                        .formatted(ticket.getId(), ticket.getTechnician().getName()),
                ticket.getId(),
                null);
    }
    ...
    public void ticketPriorityChanged(Ticket ticket, TicketPriority previousPriority, Long actorUserId) {
        ...
        publish(
                ...
                "Ticket #%d priority changed from %s to %s"
                        .formatted(ticket.getId(), previousPriority, ticket.getPriority()),
                ticket.getId(),
                null);
    }
    ...
    public void ticketCommentCreated(TicketComment comment, Long actorUserId) {
        ...
        if (ticket.getTechnician() == null
                && actorUserId != null
                && actorUserId.equals(ticket.getClient().getId())) {
        ...
        publish(
                ...
                "New ticket comment",
                "A new comment was added to ticket #%d".formatted(ticket.getId()),
                ticket.getId(),
                comment.getId());
    }
    ...
    private Set<Long> staffPool() {
        Set<Long> recipients = new LinkedHashSet<>();
        userRepository
                .findByRoleInAndActiveTrue(List.of(Roles.ROLE_ADMIN, Roles.ROLE_TECHNICIAN))
    ...
    private void publish(
        ...
        recipients.stream()
                ...
                .forEach(recipientId -> notificationOutboxService.enqueue(new NotificationMessage(
                        idGenerator.nextUuid(), recipientId, type, title, message, ticketId, commentId, actorUserId)));
    }
}
