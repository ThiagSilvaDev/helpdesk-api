...
public final class TicketSpecification {
    ...
    public static Specification<com.thiagsilvadev.helpdesk.entity.ticket.Ticket> withCriteria(TicketSearchCriteria criteria) {
        if (criteria == null) {
    ...
    private static Specification<com.thiagsilvadev.helpdesk.entity.ticket.Ticket> hasStatus(TicketStatus status) {
        return status == null
                ? Specification.unrestricted()
                : (root, query, cb) -> cb.equal(root.get(STATUS_FIELD), status);
    }

    private static Specification<com.thiagsilvadev.helpdesk.entity.ticket.Ticket> hasPriority(TicketPriority priority) {
