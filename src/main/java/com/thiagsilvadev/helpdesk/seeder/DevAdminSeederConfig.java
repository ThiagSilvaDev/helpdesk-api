...
public class DevAdminSeederConfig {
    ...
    @Bean
    CommandLineRunner devAdminSeeder(
    ...
    private Map<String, User> seedUsers(
        ...
        List<SeedUserSpec> usersToSeed = List.of(
                ...
                new SeedUserSpec(
                        "Alicia Customer", "alicia.customer@helpdesk.local", "User@123456", Roles.ROLE_USER, true),
                ...
                new SeedUserSpec(
                        "Diego Field Tech", "diego.tech@helpdesk.local", "Tech@123456", Roles.ROLE_TECHNICIAN, true),
                new SeedUserSpec(
                        "Erika Former Client", "erika.former@helpdesk.local", "User@123456", Roles.ROLE_USER, false));

        Map<String, User> usersByEmail = new HashMap<>();
        for (SeedUserSpec spec : usersToSeed) {
            User user = userRepository.findByEmail(spec.email()).orElseGet(() -> {
                log.info("Seeding development user {}", spec.email());
                User newUser =
                        new User(spec.name(), spec.email(), passwordEncoder.encode(spec.password()), spec.role());
                if (!spec.active()) {
                    newUser.deactivate();
                }
    ...
    private Map<String, Ticket> seedTickets(
        ...
        Set<String> existingTitles =
                existingTickets.stream().map(Ticket::getTitle).collect(Collectors.toSet());
    ...
    private void seedCommentsAndNotifications(
            TicketCommentRepository ticketCommentRepository,
            NotificationRepository notificationRepository,
        ...
        TicketComment comment = ticketCommentRepository.findByTicketId(ticket.getId(), Pageable.unpaged()).stream()
        ...
        UUID sourceEventId =
                UUID.nameUUIDFromBytes(("dev-seed:notification:ticket-comment-created:" + commentSpec.ticketTitle())
        ...
        User recipient =
                ticket.getTechnician() != null ? ticket.getTechnician() : usersByEmail.get("tech@helpdesk.local");
